...
class TimeBytesOptimizedGrouperPolicy(GrouperPolicy):
    def __init__(   self, 
                    grouper_data,
                    video_data,
                    video_obj,
        ...
        try:
            ...
            self.logger.info("Time-Bytes optimizer grouper created")
            ...
            self.target_length_seconds = float(grouper_data[K_PARAMETERS_GROUPER][K_TARGET_SEGMENT_LENGTH_GROUPER])
    ...
    def make_groups(self):
        ...
        self.logger.info("Starting grouping routine. Method: time  -bytes optimizer.")
        self.logger.info("Desired length-like bytes -> {}".format(self.target_bytes_length_seconds))
        self.logger.info("Desired time -> {}".format(self.target_length_seconds))
        ...
        log_file_optimizer = os.path.join(self.logs_dir, 'time_bytes_optimizer.logs')
        self.logger.debug("Logging optimization output to {}".format(log_file_optimizer))
        
        
        self.logger.info("Creating time-bytes optimizer")
        grouper_time_bytes_optimizer = GrouperTimeBytesOptimizer(   self.opt_lookahead, fragments, 'Grouper Time-Bytes Optimizer', 
                                                                    log_file_optimizer, self.target_length_seconds, 
                                                                    target_bytes_per_segment, 
                                                                    verbose=self.verbose)
        self.logger.info("Time-Bytes optimizer object created succesfully")

        
        self.logger.info("Starting time-bytes optimization")
        grouper_time_bytes_optimizer.compute_suboptimal()
        self.logger.info("Time-Bytes optimization computed succesfully")


        self.logger.debug("Retrieving keyframes boundaries and creating rescaling method")
        self.segments_keys_indexes, self.segments_keys_timestamps = grouper_time_bytes_optimizer.return_suboptimal()
